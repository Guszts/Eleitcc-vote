...
import { UserCircle, ShieldCheck, CheckCircle2, Vote as VoteIcon, LogIn } from 'lucide-react';
...
export function Votar() {
  ...
  const handleVote = async () => {
    ...
    try {
      // voterName we can pass the email, or a generic 'Eleitor'
      await addVote(user?.email || 'Eleitor Autenticado', selectedCandidate);
      setConfirmModal(false);
      setSuccess(true);
      setTimeout(() => {
        navigate('/');
  ...
  if (!user) {
    return (
      <div className="p-8 text-center max-w-lg mx-auto mt-20">
        ...
        <p className="text-gray-600 text-lg mb-8">Para garantir que apenas eleitores reais participem com 1 único voto, precisamos que você faça login.</p>
        <button
    ...
    <div className="p-4 sm:p-8 space-y-12 pb-24 max-w-4xl mx-auto">
      <section className="text-center space-y-4 pt-4">
        ...
        <p className="text-lg text-gray-600 max-w-2xl mx-auto font-medium">
          O seu voto vai definir o futuro. Olá, <b>{user.email}</b>. Escolha com sabedoria! Seu voto é único.
        </p>
      </section>
        ...
        <div className="space-y-8 text-center pb-4">
           ...
           <div className="bg-orange-50 p-4 rounded-2xl border border-orange-100">
             <p className="text-orange-800 font-bold text-sm">
               Atenção: O voto não pode ser desfeito após a confirmação. Cada usuário tem direito a apenas 1 voto.
             </p>
           </div>
